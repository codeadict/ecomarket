...
from articles.models import Article, Tag, BlogCategory as Category
...
class CategoryFeed(Feed, SiteMixin):
    ...
    def title(self, obj):
        return "%s: Newest Articles Categorized '%s'" % (self.site.name, obj.name)

    def link(self, obj):
        return obj.get_absolute_url()

    def description(self, obj):
        return "Articles Categorized '%s'" % obj.name
...
class CategoryFeedAtom(CategoryFeed):
    feed_type = Atom1Feed