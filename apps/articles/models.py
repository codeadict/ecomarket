...
from main.propertycache import cached_property
...
class BlogCategory(models.Model):
    title = models.CharField(_('title'), max_length=64, unique=True, db_index=True)
    slug = AutoSlugField(populate_from='title')
    product_category = models.ForeignKey("marketplace.Category", limit_choices_to={'parent': None}, related_name="blog_categories")
...
class Article(models.Model):
    ...
    slug = models.SlugField(_('SEO slug'), unique_for_year='publish_date')
    status = models.ForeignKey(ArticleStatus, default=ArticleStatus.objects.default)
    author = models.ForeignKey(User)
    custom_unique_id = models.CharField(max_length=100, blank=True, null=True)

    keywords = models.TextField(_('SEO meta keywords'), blank=True, help_text=_("If omitted, the keywords will be the same as the article tags."))
    description = models.TextField(_('SEO description'), blank=True, help_text=_("If omitted, the description will be determined by the first bit of the article's content."))

    markup = models.CharField(max_length=1, choices=MARKUP_OPTIONS, default=MARKUP_DEFAULT, help_text=MARKUP_HELP)
    content = RichTextField()
    rendered_content = models.TextField()

    category = models.ForeignKey(BlogCategory, blank=True, null=True, related_name="articles")
    ...
    related_products = models.ManyToManyField('marketplace.Product', blank=True)

    publish_date = models.DateTimeField(default=datetime_now, help_text=_('The date and time this article shall appear online.'))
    ...
    created_at = models.DateTimeField(_('Created at'), auto_now_add=True, default=datetime.now)
    updated_at = models.DateTimeField(_('Updated at'), auto_now=True, default=datetime.now)

    seo_keyword = models.CharField(_('SEO target keyword/phrase '), max_length=255, blank=True, null=True)
    seo_meta_title = models.CharField(_('SEO meta title'), max_length=255, blank=True, null=True)
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.id:
            ...
            if self.expiration_date and self.expiration_date <= datetime_now() and self.is_active:
...
#        return False
    ...
    def _get_youtube_video(self):
        soup = BeautifulSoup(self.rendered_content)
        for ob in soup.findAll("object"):
            embed = ob.find("embed", type="application/x-shockwave-flash")
            if embed:
                return dict(object_html=unicode(ob),
                            embed_html=unicode(embed),
                            url=embed.get("src"),
                            width=ob.get("width"),
                            height=ob.get("height"))
    youtube_video = cached_property(_get_youtube_video)
    ...
    def get_image(self):
        for attachment in self.attachments.filter(is_featured=True):
...
class Attachment(models.Model):
    upload_to = lambda inst, fn: 'attach/%s/%s/%s' % (datetime_now().year, inst.article.slug, fn)
    ...
    is_featured = models.BooleanField(_('Is Featured'), blank=True, default=False, null=False)
