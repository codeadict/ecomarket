...
from articles.models import Article, Tag, BlogCategory
...
def display_category_page(request, category=None, username=None, year=None,
                          month=None, page=1):
    """
    ...
    if category:
        category = get_object_or_404(BlogCategory, slug__iexact=category)
        articles = category.articles.live(user=request.user).select_related()
...
def display_blog_page(request, tag=None, username=None, year=None, month=None,
                      page=1):
    """
    Handles all of the magic behind the pages that list articles in any way.
    Yes, it's dirty to have so many URLs go to one view, but I'd rather do that
    ...
    if tag:
        ...
        year = int(year)
        month = int(month)
        articles = Article.objects.live(user=request.user).select_related()
...
def display_article(request, year, slug,
                    template='articles/article_detail.html'):
    """Displays a single article."""

    article = get_object_or_404(Article.objects.live(user=request.user),
                                slug=slug)
    if str(article.publish_date.year) != year:
