...
from models import Tag, Article, ArticleStatus, Attachment, BlogCategory
...
class ArticleAdmin(admin.ModelAdmin):
    list_display = ('title', 'status', 'author', 'tag_count', 'publish_date',
                    'expiration_date', 'custom_unique_id', 'is_active')
    list_filter = ('status', 'is_active', 'publish_date', 'category',
                   'expiration_date')
    list_per_page = 25
    search_fields = ('title', 'keywords', 'description', 'content')
    date_hierarchy = 'publish_date'
    form = ArticleAdminForm
    # Enable this to allow for attachments
    inlines = [
         AttachmentInline,
    ]

    fieldsets = (
        (None, {'fields': ('author', 'title', 'slug', 'content', 'category', 'status', 'custom_unique_id')}),
        ('Metadata', {
            'fields': ('seo_keyword', 'seo_meta_title', 'keywords', 'description'),
        }),
