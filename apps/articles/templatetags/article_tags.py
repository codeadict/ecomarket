...
class GetBlogCategoriesNode(template.Node):
    """
    ...
    def render(self, context):
        tags = BlogCategory.objects.all().order_by('title')
        context[self.varname] = tags
        return ''

def get_article_categories(parser, token):
    """
    Retrieves a list of live article categories and places it into the context
    """
    ...
    return GetBlogCategoriesNode(args[2])
...
def html_to_text(value):
    """Returns the text representing the html"""
    result = html2text.html2text(value)
...
#registering filters
register.filter('html2text', html_to_text)
