...
class Migration(SchemaMigration):
    ...
    depends_on = (
            ("purchase", "0004_auto__del_field_shippingaddress_cart_stall"),
            )

    def forwards(self, orm):
        # Moving model Shipping address from purchase to accounts
        db.rename_table('purchase_shippingaddress', 'accounts_shippingaddress')

        if not db.dry_run:
                # For permissions to work properly after migrating
                orm['contenttypes.contenttype'].objects.filter(app_label='purchase', model='shipping_address').update(app_label='accounts')


    def backwards(self, orm):
        # Moving table from accounts back to purchase
        db.rename_table('accounts_shippingaddress', 'purchase_shippingaddress')
