...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'UserProfile.activation_key_date'
        db.add_column('accounts_userprofile', 'activation_key_date',
        ...
        if not db.dry_run:
            ...
            for profile in orm.UserProfile.objects.all():
                try:
                    ...
                    continue
                profile.objects.filter(id=profile.id).update(activation_key_date=profile.user.date_joined)
                counter += 1
                if self.VERBOSE and counter % 10 == 0:
                    print "Updated {} records".format(counter)
        db.alter_column('accounts_userprofile', 'activation_key_date',
                        self.gf('django.db.models.fields.DateTimeField')(auto_now_add=True, blank=True))


    def backwards(self, orm):
        # Deleting field 'UserProfile.activation_key_date'
        db.delete_column('accounts_userprofile', 'activation_key_date')
