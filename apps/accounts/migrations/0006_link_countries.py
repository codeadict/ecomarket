...
class Migration(DataMigration):
    ...
    depends_on = (
        ("marketplace", "0005_shipping_profiles"),
    )
    ...
    def forwards(self, orm):
        ...
        try:
            uk = orm['marketplace.Country'].objects.get(code="GB")
        except orm['marketplace.Country'].DoesNotExist:
            print("Country doesn't exist")
            uk = orm['marketplace.Country'].objects.create(code="GB", title="United Kingdom")

        for profile in orm.UserProfile.objects.all():
            try:
                country = orm['marketplace.Country'].objects.get(code=profile.country)
                profile.country_new = country
                profile.save()
            except orm['marketplace.Country'].DoesNotExist:
                print("Country not found for user {0}".format(profile.user.username))
                profile.country_new = uk
                profile.save()
    ...
    models = {
        ...
        'accounts.userprofile': {
            ...
            'country': ('django_countries.fields.CountryField', [], {'default': "'GB'", 'max_length': '2'}),
            'country_new': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['marketplace.Country']", 'null': 'True'}),
