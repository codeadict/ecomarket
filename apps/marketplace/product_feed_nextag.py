...
# Nextag uses category IDs from Shopzilla, so we reuse this mapping from Shopzilla code
...
class TSVProductFeedBuilder(XMLProductFeedBuilder):
    ...
    def generate_description(self):
        return self.product.description[0:500]. \
            replace('\t', ' ').replace('\n\r', ' ').replace('\r\n', ' ').replace('\n', ' ').replace('\r', ' ')
    ...
    def generate(self):
        ...
        if self.country.code == 'GB':
            ...
            item = list([
                ...
                self.generate_id(), # Merchant Part Number
                self.generate_title(), # Product Name
                self.generate_description(),
                self.generate_link(), # Click-Out URL
                self.generate_price(),
                self.generate_category(), # Category: Other Format
                self.generate_category_id(), # Category: Shopzilla Numeric ID
                self.generate_image(),
                self.generate_shipping(), # Ground Shipping
                self.generate_availability(), # Stock Status
                self.generate_condition() # Product Condition
            ])
...
class TSVFeedWriter(XMLFeedWriter):
    ...
    def write(self, fh):
        tsvh = csv.writer(fh, dialect='excel')
        if self.country.code == 'GB':
            tsvh.writerow(['Manufacturer', 'Merchant Part Number',
                'Product Name', 'Description', 'Click-Out URL', 'Price',
                'Category: Other Format', 'Category: Shopzilla Numeric ID',
                'Image URL', 'Ground Shipping', 'Stock Status', 'Condition'])
