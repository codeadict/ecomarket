...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        models = [(orm.Recipient, "recipients"), (orm.Material, "materials"),
                  (orm.SuggestedCertificate, "suggested_certificates"),
                  (orm.Color, "colors"), (orm.Certificate, "certificates"),
                  (orm.Cause, "causes"), (orm.Keyword, "keywords"),
                  (orm.Ingredient, "ingredients"), (orm.Occasion, "occasions")]
        for model, accessor in models:
            for ob in model.objects.all():
                duplicates = \
                    model.objects.filter(title=ob.title).exclude(id=ob.id)
                if duplicates.count():
                    duplicate = duplicates[0]
                    if model is orm.SuggestedCertificate:
                        products = [ob.product]
                    else:
                        products = ob.products.all()
                    for product in products:
                        getattr(product, accessor).add(duplicate)
                    if model is orm.Cause:
                        for cert in ob.certificates.all():
                            duplicate.certificates.add(cert)
                    ob.delete()

    def backwards(self, orm):
        warnings.warn("Cannot reverse this migration. Doing nothing...")
