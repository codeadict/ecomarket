...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Keywords have been accidentally created which contain commas rather
        than being multiple individual keyword objects. To get around this we
        split the name by comma and create new keyword objects, then link them
        to the relevant products.
        """
        # Bizarrely some of the problematic keywords have angle brackets around
        # title.
        angleReg = re.compile("<|>")
        for keyword in orm.Keyword.objects.filter(title__contains=","):
            ...
            for name in names:
                try:
                    new_keyword = orm.Keyword.objects.get(title=name)
                except orm.Keyword.DoesNotExist:
                    new_keyword = orm.Keyword.objects.create(title=name)
