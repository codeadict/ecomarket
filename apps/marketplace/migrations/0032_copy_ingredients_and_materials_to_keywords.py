...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Copy ingredients and materials into new keyword objects"""
        ingredients = orm.Ingredient.objects.all()
        materials = orm.Material.objects.all()
        for obj in itertools.chain(ingredients, materials):
            print("Processing object {0}".format(obj.title))
            try:
                keyword = orm.Keyword.objects.get(title=obj.title)
            except orm.Keyword.DoesNotExist:
                keyword = orm.Keyword.objects.create(title=obj.title)
