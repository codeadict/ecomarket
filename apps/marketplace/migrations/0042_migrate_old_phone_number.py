...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for stall in orm.Stall.objects.exclude(phone_number=''):
            orm.StallOldStylePhoneNumber.objects.create(
                stall=stall, phone_number=stall.phone_number)

    def backwards(self, orm):
        "Write your backwards methods here."
        for old_style in orm.StallOldStylePhoneNumber.objects.all():
            orm.Stall.objects.filter(id=old_style.stall_id).update(
                    phone_number=old_style.phone_number)
            old_style.delete()
