...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        try:
            filename, category_slug = args
        except ValueError:
            raise CommandError(
                "This command takes 2 arguments: the filename to write to and category slug to filter")
        ...
        lines = {}
        for row in LineItem.objects.values('product_id', 'id'): lines[row['product_id']] = row['id']
        products = list()
        count = 0
        for product in all_products:
            try:
                if product.status == Product.PUBLISHED_LIVE:
                    category_slugs = [cat.slug for cat in product.primary_category.get_ancestors()]
                    #print category_slugs
                    if category_slug in category_slugs:
                        product.order_weight = 0
                        if product.id in lines:
                            product.order_weight = lines[product.id]
                        products.append(product)
            except:
                pass
        print "DEBUG: %s Results" % len(products)

        products.sort(key=lambda x: x.order_weight, reverse=True)
