...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'Payment.cart_stall'
        db.delete_column('purchase_payment', 'cart_stall_id')

        # Adding field 'Payment.order'
        db.add_column('purchase_payment', 'order',
                      self.gf('django.db.models.fields.related.OneToOneField')(default=1, related_name='payment', unique=True, to=orm['purchase.Order']),
                      keep_default=False)


    def backwards(self, orm):

        # User chose to not deal with backwards NULL issues for 'Payment.cart_stall'
        raise RuntimeError("Cannot reverse this migration. 'Payment.cart_stall' and its values cannot be restored.")
        # Deleting field 'Payment.order'
        db.delete_column('purchase_payment', 'order_id')
